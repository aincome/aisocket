...
io.on("connection", (socket) => {
  socket.on("input", async (input) => {
    const prompt =
      process.env.AISOCKET_PROMPT?.replace("${input}", input) || input;
    let response = "";
